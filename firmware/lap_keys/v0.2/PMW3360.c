...
#define BEGIN_COM SS_LOW(_ss); wait_us(1)
#define END_COM   wait_us(1); SS_HIGH(_ss)
...
/* 
set_cpi: set CPI level of the motion sensor.
...
{  
  uint16_t fromLast = micros() - _lastBurst;
  ...
  uint8_t buf0 = SPI_ReadByte();
  SPI_SendByte(0x00);
  data.isMotion = (buf0 & 0x80) != 0;
  data.isOnSurface = (buf0 & 0x08) == 0;   // 0 if on surface / 1 if off surface

  data.dx = SPI_ReadByte();         // dx LSB
  data.dx |= (SPI_ReadByte() << 8); // dx MSB
  data.dy = SPI_ReadByte();         // dy LSB
  data.dy |= (SPI_ReadByte() << 8); // dy MSB
  data.shutter = SPI_ReadByte();         // shutter LSB
  data.shutter |= (SPI_ReadByte() << 8); // shutter MSB
  data.SQUAL = SPI_ReadByte();
  data.rawDataSum = SPI_ReadByte();
  data.maxRawData = SPI_ReadByte();
  data.minRawData = SPI_ReadByte();
